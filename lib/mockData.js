...
export function generateMockForRep(seed = 'default', { includeLandmark = true } = {}) {
  ...
  // Generate descending amounts seeded per rep — largest contributor first
  const topAmount = Math.round((120_000 + rng() * 380_000) / 1000) * 1000;
  const topContributors = pool.map((c, i) => {
    // Each subsequent contributor gets 40–80% of the previous one's amount
    const decayFactor = 0.4 + rng() * 0.4;
    const amount = i === 0 ? topAmount : Math.round((topAmount * Math.pow(decayFactor, i)) / 500) * 500;
    return { ...c, amount: Math.max(amount, 5000) };
  });
...
// ============================================================
// State-level bill pool — used exclusively for state/local rep mock data.
// Every title here is plausible state legislation; nothing federal.
// ============================================================
const STATE_BILLS = [
  // Healthcare (state programs, Medicaid, public health)
  { billTitle: 'Medicaid Expansion Enhancement Act',                  category: 'Healthcare' },
  { billTitle: 'Mental Health Services in Public Schools Act',        category: 'Healthcare' },
  { billTitle: 'Rural Health Clinic Funding Act',                     category: 'Healthcare' },
  { billTitle: 'Prescription Drug Price Transparency Act',            category: 'Healthcare' },
  { billTitle: "Children's Dental and Vision Coverage Act",           category: 'Healthcare' },
  { billTitle: 'Community Health Worker Program Act',                 category: 'Healthcare' },
  // Environment (state environmental agencies, state parks, etc.)
  { billTitle: 'Clean Water Quality Standards Act',                   category: 'Environment' },
  { billTitle: 'State Renewable Energy Portfolio Standards Act',      category: 'Environment' },
  { billTitle: 'Climate Resilience and Planning Act',                 category: 'Environment' },
  { billTitle: 'Solid Waste Reduction and Recycling Incentive Act',   category: 'Environment' },
  { billTitle: 'State Park Land Preservation Act',                    category: 'Environment' },
  // Economy (state taxes, local businesses, infrastructure)
  { billTitle: 'Small Business Development and Tax Relief Act',       category: 'Economy' },
  { billTitle: 'State Infrastructure Maintenance and Repair Act',     category: 'Economy' },
  { billTitle: 'Workforce Training and Apprenticeship Act',           category: 'Economy' },
  { billTitle: 'Local Hiring and Economic Development Act',           category: 'Economy' },
  { billTitle: 'Affordable Housing Construction Incentive Act',       category: 'Economy' },
  { billTitle: 'Tenant Rights and Eviction Protection Act',           category: 'Economy' },
  { billTitle: 'First-Time Homebuyer Assistance Program Act',         category: 'Economy' },
  // Education (K-12, community colleges, state universities)
  { billTitle: 'Public School Funding Equity Act',                    category: 'Education' },
  { billTitle: 'Early Childhood Education Expansion Act',             category: 'Education' },
  { billTitle: 'Teacher Recruitment and Retention Act',               category: 'Education' },
  { billTitle: 'Community College Tuition Affordability Act',         category: 'Education' },
  { billTitle: 'Career and Technical Education Investment Act',       category: 'Education' },
  { billTitle: 'Special Education Funding Adequacy Act',              category: 'Education' },
  // Energy (state utilities, efficiency standards)
  { billTitle: 'State Energy Efficiency Standards Act',               category: 'Energy' },
  { billTitle: 'Utility Rate Transparency and Consumer Protection Act', category: 'Energy' },
  { billTitle: 'Electric Vehicle Charging Infrastructure Act',        category: 'Energy' },
  // Other (public safety, courts, local government)
  { billTitle: 'Criminal Justice Diversion and Rehabilitation Act',   category: 'Other' },
  { billTitle: 'Community Policing Grant Program Act',                category: 'Other' },
  { billTitle: 'Victims Compensation and Support Services Act',       category: 'Other' },
  { billTitle: 'Local Government Transparency and Ethics Act',        category: 'Other' },
];

/**
 * Generate deterministic mock legislative data for a STATE or LOCAL legislator.
 * Uses only state-appropriate bill names; never includes federal landmark votes.
 *
 * @param {string} seed  - Stable identifier (OpenStates ID or name+state)
 */
export function generateMockForStateLeg(seed = 'default') {
  const rng = seededRng(seed);

  const totalBills = 7 + Math.floor(rng() * 5);
  const shuffled   = [...STATE_BILLS].sort(() => rng() - 0.5).slice(0, totalBills);
  const sponsoredCount   = Math.max(2, Math.floor(totalBills * 0.35));
  const cosponsoredCount = Math.floor(totalBills * 0.30);

  // State bill identifiers vary by chamber and state convention
  const prefixes = ['SB', 'AB', 'HB', 'SB', 'HB'];   // SB/HB more common
  const prefix    = pick(rng, prefixes);
  const numBase   = 100 + Math.floor(rng() * 800);

  const bills = [
    ...shuffled.slice(0, sponsoredCount).map((b, i) => ({
      billId:     `${prefix} ${numBase + i}`,
      billTitle:  b.billTitle,
      category:   b.category,
      vote:       'Sponsored',
      weight:     1.0,
