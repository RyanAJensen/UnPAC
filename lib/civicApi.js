...
async function getRepsAtLocation(lat, lng) {
  ...
  // Retry once on 502/503 (transient OpenStates errors)
  let res = await fetch(url, { headers: { 'X-API-KEY': process.env.OPENSTATES_API_KEY ?? '' } });
  if ((res.status === 502 || res.status === 503) ) {
    await new Promise(r => setTimeout(r, 1500));
    res = await fetch(url, { headers: { 'X-API-KEY': process.env.OPENSTATES_API_KEY ?? '' } });
  }
